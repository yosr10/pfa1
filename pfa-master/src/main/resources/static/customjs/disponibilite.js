 /********************* Edit anneeAcademique********************************/
   function loadDisponibilite(field){ //open modal
   		var data = field.closest("td").id;
   		$('#idEdit').val(data);
    	$('#jourEdit').val($('#jour' + data).text());
    	$('#creneauEdit').val($('#creneau' + data).text());
    	
    	$('#edit-disponibilite-modal').modal('show');
    }
    $( "#disponibilite-edit-form" ).submit(function(event ) { //edit action
    	event.preventDefault();
    	var data = $('#idEdit').val();
    	$.ajax({
			url : '/admin/Disponibilites/update',
			type : 'POST',
			data :  $("#disponibilite-edit-form").serialize(),
			...
			success : function(response) {	
		    	...
		    	$('#edit-disponibilite-modal').modal('hide');
		    	swal("Updated!", "The disponibilite has been updated.", "success");
			}
		});
    });
    /*********************New anneeAcademique********************************/
    function newDisponibilite(){ //open modal
    	$('#jourNew').val('');
    	$('#creneauNew').val('');
    	
    	$('#new-disponibilite-modal').modal('show');
    }
  /*  $( "#anneeAcademique-new-form" ).submit(function(event ) { //add action
   ...
   function deleteDisponibilite(field){
   var data = field.closest("td").id;
        swal({   
            title: "Are you sure?",   
    		...
    		$.ajax({
    			url : '/admin/Disponibilites/delete',
    			type : 'POST',
    			data : {
    				id : data
    			},
